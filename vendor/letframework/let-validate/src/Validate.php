...
class Validate
{
    ...
    {
        return isset($int) ? filter_var($int, FILTER_VALIDATE_INT, [
            'options' => [
                'default' => $default,
                'min_range' => $min,
                'max_range' => $max
    ...
    {
        return isset($float) && $float >= $min && $float <= $max ? filter_var($float, FILTER_VALIDATE_FLOAT, [
            'options' => [
                'default' => $default,
                'decimal' => $decimal
            ]
    ...
    {
        return isset($bool) ? filter_var($bool, FILTER_VALIDATE_BOOLEAN, [
            'options' => [
                'default' => $default
            ]
        ]) : $default;
    ...
    {
        return isset($url) ? filter_var($url, FILTER_VALIDATE_URL, [
            'options' => [
                'default' => $default
            ],
