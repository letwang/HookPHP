...
function init(string $appName = APP_NAME)
{
    ...
    $data = [];
    foreach ($pdo->fetchAll(str_replace('?', '"'.APP_CONFIG['application']['prefix'] . $appName . '_%"', apcu_fetch('global')['sql']['TABLE']['GET_ALL']), [], PDO::FETCH_NUM) as list($table)) {
    ...
    $result &= file_put_contents(getcwd() . '/app/' . $appName . '/config/table.php', '<?php' . PHP_EOL . 'declare(strict_types=1);' . PHP_EOL. PHP_EOL . 'return ' . str_replace(['array (', ')'], ['[', ']'], var_export($data, true)) . ';') > 0;
