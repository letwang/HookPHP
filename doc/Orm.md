...
$orm->select()
	...
	->limit(30, 30)
	->fetchAll()); #SELECT `id` FROM `hp_config` WHERE (`date_add`=?) AND (`app_id`<? OR `status`=? OR `id`IN(?,?,?)) AND (`date_add`=?) GROUP BY `app_id` ASC, `id` DESC ORDER BY `app_id` ASC, `id` DESC LIMIT 30,30
```
# 插入
```
$data = $orm->insert([
