...
echo "PHP version               : ${PHPCLI_VERSION}"
echo "Extra Extensions          : ${PHPCLI_EXTENSIONS}"
...
if [[ -z "${EXTENSIONS##*,pdo_sqlsrv,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        echo "pdo_sqlsrv requires PHP >= 7.1.0, installed version is ${PHPCLI_VERSION}"
    fi
fi
